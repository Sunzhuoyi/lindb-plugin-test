...
// Types
import { LindbContext, LindbOptions, LindbQuery, LindbQueryRequest } from './types';
...
export class LindbDatasource extends DataSourceApi<LindbQuery, LindbOptions> {
  type: string;
  editorSrc: string;
  ...
  constructor(instanceSettings: DataSourceInstanceSettings<LindbOptions>) {
  ...
  getQueryDisplayText(query: LindbQuery) {
    return query.expr;
  }

  _addTracingHeaders(httpOptions: LindbQueryRequest, options: DataQueryRequest<LindbQuery>) {
    httpOptions.headers = {};
  ...
  targetContainsTemplate(target: LindbQuery) {
    return templateSrv.variableExists(target.expr);
  }

  processResult = (response: any, query: LindbQueryRequest, target: LindbQuery, responseListLength: number) => {
  ...
  prepareTargets = (options: DataQueryRequest<LindbQuery>, start: number, end: number) => {
    const queries: LindbQueryRequest[] = [];
    const activeTargets: LindbQuery[] = [];
    ...
    for (const target of options.targets) {
      ...
      if (target.context !== LindbContext.Explore) {
  ...
  calledFromExplore = (options: DataQueryRequest<LindbQuery>): boolean => {
    const exploreTargets = options.targets.filter(target => target.context === LindbContext.Explore).length;
    return exploreTargets === options.targets.length;
  };

  query(options: DataQueryRequest<LindbQuery>): Observable<DataQueryResponse> {
  ...
  private exploreQuery(queries: LindbQueryRequest[], activeTargets: LindbQuery[], end: number) {
  ...
  private panelsQuery(queries: LindbQueryRequest[], activeTargets: LindbQuery[], end: number, requestId: string) {
  ...
  createQuery(target: LindbQuery, options: DataQueryRequest<LindbQuery>, start: number, end: number) {
    const query: LindbQueryRequest = {
  ...
  performTimeSeriesQuery(query: LindbQueryRequest, start: number, end: number) {
    if (start > end) {
  ...
  performInstantQuery(query: LindbQueryRequest, time: number) {
  ...
  handleErrors = (err: any, target: LindbQuery) => {
  ...
  async testDatasource() {
    ...
    const query = { expr: '1+1' } as LindbQueryRequest;
  ...
  interpolateVariablesInQueries(queries: LindbQuery[]): LindbQuery[] {
  ...
  getQueryHints(query: LindbQuery, result: any[]) {
  ...
  modifyQuery(query: LindbQuery, action: any): LindbQuery {
